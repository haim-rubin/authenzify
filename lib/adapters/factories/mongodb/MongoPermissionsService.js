...
class MongoPermissionsService {
    ...
    async findGroup({ id, tenantId, }) {
        const group = await __classPrivateFieldGet(this, _MongoPermissionsService_modelsCollections, "f").PermissionsGroup.findOne({
            tenantId,
            id,
        });
        return group;
    ...
    async findGroupsByTenantId(tenantId) {
        const group = await __classPrivateFieldGet(this, _MongoPermissionsService_modelsCollections, "f").PermissionsGroup.find({
            tenantId,
        });
        return (0, mongodb_util_1.mapMongoDbId)(group);
    }
    async createGroup(permissionsGroupDetails) {
        const permissionsGroup = await __classPrivateFieldGet(this, _MongoPermissionsService_modelsCollections, "f").PermissionsGroup.create(permissionsGroupDetails);
        return permissionsGroup;
    }
    async findGroups({ tenantId, filter, }) {
        const permissionsGroups = await __classPrivateFieldGet(this, _MongoPermissionsService_modelsCollections, "f").PermissionsGroup.find({
            ...filter,
            tenantId,
        });
    ...
    async deleteGroup({ id, tenantId }) {
        const permissionsGroup = await __classPrivateFieldGet(this, _MongoPermissionsService_modelsCollections, "f").PermissionsGroup.updateOne({
            id,
            tenantId,
        }, { isDeleted: true });
        return permissionsGroup;
    ...
    async getTransaction() {
        const transaction = await __classPrivateFieldGet(this, _MongoPermissionsService_modelsCollections, "f").User.startSession();
        return transaction;
    }
