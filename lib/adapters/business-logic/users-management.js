...
class UsersManagement {
    ...
    async requestPermissionForUser({ id, companyDetails, userInfo }) {
        ...
        if (userInfo.email === companyDetails.email && !user.tenantId) {
            ...
            const updatedUserResponse = await __classPrivateFieldGet(this, _UsersManagement_services, "f").Users.updateUser({ id }, { tenantId });
            const permissionsGroups = await this.createPermissionsGroupsForNewCompany({ tenantId });
        }
        ...
        const verification = await this.createVerification({
            userId: id,
            type: constant_1.VERIFICATION_TYPES.PERMISSIONS,
        });
